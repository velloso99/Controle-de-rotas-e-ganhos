...
def criar_dados():
    with con:
        cur = con.cursor()
        query = "INSERT INTO Rota_Mercado_Livre(data,valor_rota,valor_bomba,km,valor_total,lucro,entregas,devolvidas,dia_semana) values(?,?,?,?,?,?,?,?)"
        cur.execute(query, i)
...
def atualizar_dados(i):
    with con:
        cur = con.cursor()
        query = "UPDATE Rota_Mercado_Livre SET data=?,valor_rota=?,valor_bomba=?,km=?,valor_total=?,lucro=?,entregas=?,devolvidas=? WHERE id=?"
        cur.execute(query, i)
#-----------------------------------------------------------------------------------------------------------------  
def excluir_dados(i):
    with con:
